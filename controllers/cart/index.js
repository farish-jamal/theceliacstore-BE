...
const addToCart = asyncHandler(async (req, res) => {
  ...
  if (!type || !quantity || quantity <= 0) {
    return res.status(400).json(
      new ApiResponse(400, null, "Invalid request: type and quantity are required", false)
    );
  }

  if (type === "product" && !product_id) {
    return res.status(400).json(
      new ApiResponse(400, null, "Product ID is required for product type", false)
    );
  }

  if (type === "bundle" && !bundle_id) {
    return res.status(400).json(
      new ApiResponse(400, null, "Bundle ID is required for bundle type", false)
    );
  }
  ...
  if (!cartItem) {
    res.json(
      new ApiResponse(200, null, "Cart is empty", true)
    );
  } else {
    res.json(
